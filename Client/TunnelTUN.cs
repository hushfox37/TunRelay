...
namespace TunRelayClient
{
    ...
    public class WintunDriver : ITunDriver
    {
        ...
        private static unsafe void CopyToNative(ReadOnlySpan<byte> src, IntPtr dst)
        {
            ...
            src.CopyTo(dest);
            FixIPv4Checksums(dest);
        }

        private static void FixIPv4Checksums(Span<byte> packet)
        {
            ...
            packet = packet.Slice(0, totalLength);

            packet[10] = 0;
            packet[11] = 0;
            BinaryPrimitives.WriteUInt16BigEndian(packet.Slice(10, 2), FoldChecksum(AddWords(0, packet.Slice(0, headerLength))));

            ushort fragment = BinaryPrimitives.ReadUInt16BigEndian(packet.Slice(6, 2));
            ...
            int minTransportLength = protocol == Protocol.TCP ? 20 : 8;
            if (transportLength < minTransportLength)
                return;

            int checksumOffset = headerLength + (protocol == Protocol.TCP ? 16 : 6);
            if (protocol == Protocol.UDP && BinaryPrimitives.ReadUInt16BigEndian(packet.Slice(checksumOffset, 2)) == 0)
                return;
            ...
            sum = AddWords(sum, packet.Slice(headerLength, transportLength));

            ushort checksum = FoldChecksum(sum);
            if (protocol == Protocol.UDP && checksum == 0)
                checksum = 0xFFFF;

            BinaryPrimitives.WriteUInt16BigEndian(packet.Slice(checksumOffset, 2), checksum);
        }
