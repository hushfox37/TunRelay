...
namespace TunRelayClient
{
    ...
    internal sealed class ControlHandshake
    {
        ...
        public async Task<ControlSession> ConnectWithRetryAsync(
            ...
            ServerAssignment? expectedAssignment = null)
        {
            while (true)
            {
                TunnelNet? net = null;
                try
                {
                    ...
                    if (expectedAssignment != null)
                        assignment.EnsureCompatibleWith(expectedAssignment);
                ...
                catch (DataChannelConfigurationException)
                {
                    net?.Close();
                    throw;
                }
        ...
        private async Task<bool> AuthenticateAsync(TunnelNet tunnelNet, CancellationToken ct)
        {
            ...
            var json = JsonSerializer.Serialize(
                new AuthenticationRequest
                {
                    ...
                    Timestamp = timestamp,
                    Sign = sign,
                    SupportedProtocols = DataChannelProtocolCodec.SupportedWireValues
                },
            ...
            if (response.StartsWith("UnsupportedProtocol:", StringComparison.Ordinal))
                throw new DataChannelConfigurationException(response);
        ...
        {
            var json = JsonSerializer.Serialize(
                new CredentialProvisioningRequest
                {
                    Mode = "AutoCredentials",
                    ClientID = _config.ClientID,
                    SupportedProtocols = DataChannelProtocolCodec.SupportedWireValues
                },
            ...
            {
                if (payload?.Error.StartsWith("UnsupportedProtocol:", StringComparison.Ordinal) == true)
                    throw new DataChannelConfigurationException(payload.Error);
